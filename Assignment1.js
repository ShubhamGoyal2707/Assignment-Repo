function findCaliforniaCafes(substring) {
    // storing the given arrays in 2 internal variables
    const cafes = [
        {
            "name": "Bazaar Cafe",
    ...
    // Setting id as key of the array for fast access
    let placesWithKey = [] ;
    places.forEach(place => {
        placesWithKey[place.id] = place;
    });
    
    let results = [];
    //Searching substring and concatenating the data
    cafes.forEach(c => {
        if(c.name.includes(substring)){
            results.push({
                name: c.name,
                ...placesWithKey[c.place_id]
            });
        }
    });
